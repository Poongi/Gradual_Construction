...
def main():
    ...
    blackbox = torch.load("./models/HELOC_retrained_GS.pt")
    # blackbox = RandomForestClassifier(n_estimators=20)
    # blackbox.fit(X_train, y_train)

    X2E = X_test[:10]
    y2E = blackbox.predict(X2E)
    evaluate_model(blackbox, np_to_cuda(X2E), np_to_cuda(y2E), sklearn=True)
...
def minmax_unscaler(X):
    ...
    for col in range(X.shape[1]):
        X_col_max = max_list[col]
        X_col_min = min_list[col]
        rtn[:,col] = (X_col_max - X_col_min)/(rtn[:,col] - X_col_min)
    return rtn
