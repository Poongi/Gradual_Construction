...
def minmax_scaler(X):
    ...
    rtn = torch.tensor(X)
    max_list = torch.tensor(max_list).to(device)
    min_list = torch.tensor(min_list).to(device)
    rtn = (rtn-min_list)/(max_list-min_list)
...
class MLP(nn.Module):
    ...
    def forward(self, x):
        ...
        output = self.relu(self.fc3(output))  #layer3
        output = self.relu(self.fc4(output))  #layer4
        output = self.fc5(output)   #layer5
        return output
