...
test_cf = counterfactual_tmp.cpu() # Min-max scaled
...
for target in range(nbr_experiments):
    ...
    if distance_metric == 'mahalanobis':
        dist = cdist((currentX).reshape(1, -1), (test_x[filteredIdx]), 'mahalanobis')[0]
    elif distance_metric == 'manhatan':
        dist = cdist((currentX).reshape(1, -1), (test_x[filteredIdx]), 'cityblock')[0]
    else:
        dist = -1
        exit(-1)

    eps = np.percentile(dist, 5)
    filteredIdx_under_eps = filteredIdx[np.where((dist <= eps))]
