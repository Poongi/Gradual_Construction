...
evaluate_model(blackbox, X_original, y_pred_from_X)
print("predict of target, cf match ratio")
evaluate_model(blackbox, X_original, y_pred_from_cf)

device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

# variable copy
X_original_tmp = torch.tensor(X_original)
counterfactual_tmp = torch.tensor(counterfactual)
...
for target in range(nbr_experiments):
    ...
    '''
    eps = 1e-2 # Possible candidates 1e-2, 1e-3, 1e-4
    dist = []
