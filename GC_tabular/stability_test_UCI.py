...
# for i in range(7500):
#     if os.path.isdir("./result/UCI/test"+str(i)) == False:
#         continue
#     result_o_tmp = pd.read_csv("./result/UCI/test"+str(i)+"/org.csv")
#     result_c_tmp = pd.read_csv("./result/UCI/test"+str(i)+"/per.csv")
...
# print("clf result", y_pred_from_X_tmp)
...
# data load
X_test_raw = pd.read_csv('./example/UCI/UCI_dataset_backup/X_test_unscaled.csv')
X_test_raw = df_to_cuda(X_test_raw)

X_train_raw = pd.read_csv('./example/UCI/UCI_dataset_backup/X_train_unscaled.csv')
X_train_raw = df_to_cuda(X_train_raw)

y_train = pd.read_csv('./example/UCI/UCI_dataset_backup/y_train.csv')
y_train = df_to_cuda(y_train).squeeze()
y_test = pd.read_csv('./example/UCI/UCI_dataset_backup/y_test.csv')
...
for i in range(nbr_experiments):
    ...
    pd.DataFrame(cuda_to_numpy(noised_raw1_scaled)).to_csv("./example/UCI/UCI_test_set_noised/test"+str(i)+".csv", index = False)
    
    main.gradual_conscruction(i)
    if os.path.isdir("./result/UCI_stability_results/test"+str(i)) == False:
        continue
    X_test_tmp = pd.read_csv("./result/UCI_stability_results/test"+str(i)+"/org.csv")
    cf_tmp = pd.read_csv("./result/UCI_stability_results/test"+str(i)+"/per.csv")
