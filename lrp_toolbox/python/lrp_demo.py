...
cnn_net = model.Net()
cnn_net.state_dict(torch.load('/home/heedong/Documents/lrp_toolbox/python/models/saved/mnist_cnn.pt'))

# X = data_io.read('../data/MNIST/test_images.npy')
# Y = data_io.read('../data/MNIST/test_labels.npy')

img_read_path = '/home/heedong/Documents/lrp_toolbox/python/data/MNIST_paper/'
...
X_read = trans(X_read).float()

# X_read = torch.tensor((X_read-0.1317)/0.3081)

# X_read = torch.tensor(X_read/127.5 - 1)
X_read = torch.reshape(X_read, (len(read_list), 1, 28, 28))
# X_read = trans(X_read).float()
X_read = np.array(X_read)

# X_read = X_read.reshape(len(read_list),1,28,28)
_, y_pred = torch.max(torch.tensor(cnn_net(X_read)),1)
y_pred


# model(X_read)

# X = X_read.reshape(len(read_list),-1)
