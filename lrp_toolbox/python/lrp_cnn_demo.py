...
# nn = model.Net()
# nn.load_state_dict(torch.load('/home/heedong/Documents/lrp_toolbox/python/models/saved/mnist_cnn.pt'))
...
# root = './data/MNIST_paper/'
# paths = sorted(os.listdir(root))
# images = [Image.open(os.path.join(root, name)).convert('L') for name in paths]

# X_read = torch.stack([transform(image) for image in images])

# X_read = X_read.numpy()


# _, pred_Y = torch.tensor(nn.forward(X_read)).max(dim=1)

# # X_numpy = X.numpy()
# pred_Y_numpy = pred_Y.numpy()
# pd.DataFrame(pred_Y).to_csv('./data/MNIST_paper_label/pred_labe_paper.csv', index=False)
...
img_read_path = './data/MNIST_paper/'
read_list = os.listdir(img_read_path)
X_read = np.empty((len(read_list),28,28))
for i, name in enumerate(read_list):
    # X_read = np.append(X_read, plt.imread(img_read_path+i), axis=0)
    X_read[i] = Image.open(img_read_path+name).convert('L')
...
# y_read = pd.read_csv('./data/MNIST_paper_label/pred_labe_paper.csv', index_col = False)
# Y = np.array(y_read)

Y = np.array([3,9,3,8,2,7,9,7,3,8,5]).reshape(11,1)
...
for i in I[:11]:
    ...
    digit_hm = render.save_image([digit,hm],'../results/'+str(i)+'.png')
    data_io.write(R,'../results/'+str(i)+'.npy')
