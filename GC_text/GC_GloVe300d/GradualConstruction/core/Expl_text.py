...
class Expl_text(Expl_base):
    def __init__(self,model_path,data_path,d,n_iter,\
                ...
                tv_beta,tv_coeff,ref_path,saved_path, iteration):

        super(Expl_text, self).__init__(model_path, data_path, d, n_iter, lr, l2_coeff, \
                                           target_class, target_prob, tv_beta, tv_coeff, ref_path, saved_path, iteration)
        min_len=5
        nlp = spacy.load('en')
        if '.txt' in data_path:
            self.tokenized=np.loadtxt(data_path,dtype='str')
        else:
            org_data=data_path
        # if np.size(org_data) == 1:
        #     self.tokenized = [tok.text for tok in nlp.tokenizer(org_data)]
        # self.org_data_split=self.org_data(, ' ').split(' ')
        assert self.d==1
        if int(np.shape(self.tokenized)[0]) >=min_len:
            self.D=int(np.shape(self.tokenized)[0])
        else:
            self.D=min_len
    ...
    def build(self):
        ...
        while(1):
            ...
            if target_cls==0 and self.target_prob >= output_comp_s:
                ...
                break
            if mask_num==np.shape(self.tokenized)[0]:
        ...
        for i in range(len(used_feat)):
            ...
            for j in range(np.shape(self.TEXT.vocab.vectors)[0]):
                ...
                if min_val > dist:
                    if self.TEXT.vocab.itos[j] == self.tokenized[used_feat[i]]:
                        break
                    min_val=dist
                    min_index[i] = j

        composite_text=self.tokenized
        for i in range(len(used_feat)):
        ...
        with open(self.saved_path+str(self.iteration)+'.txt', 'w') as out_file:
            out_file.writelines('Original text: {}\n'.format(self.org_data))
            out_file.writelines('Composite text: {}'.format(composite_text))
        with open(self.saved_path+'mask_num_'+str(self.iteration)+'.txt', 'w') as out_file:
            out_file.write(str(mask_num))

        self.nbr_total_mask += mask_num
