...
# load model
blackbox = torch.load('./models/saved/HELOC_retrained.pt')
...
Model load
'''
lr = torch.load('./models/saved/HELOC_retrained.pt')
...
nbr_experiments = test_x.shape[0]
...
for target in range(nbr_experiments):
    ...
    '''
    eps = 1e-1 # Possible candidates 1e-2, 1e-3, 1e-4
    dist = []
